# microbiome at GENERA level

# PREP DATA FOR BETADISPER ANALYSIS

# DATA
# use the subsampled -lowest-level- and rarefyed data. In DADA2, that is the ASV table.
ps.rowsum_glom_biom.gen = readRDS(file.path(path_data, "/16S_DADA2_processed/Zoo.rowsum_gen.rds"))
ps.rowsum_glom_biom.gen


#Remove taxa 
#This protects against an OTU with small mean & trivially large C.V.
ps.rowsum.gen_biom.gen.cmm = filter_taxa(ps.rowsum_glom_biom.gen, function(x) sum(x > 0.00) > (0.01*length(x)), TRUE)
ps.rowsum.gen_biom.gen.cmm

ps.rowsum_biom.gen = ps.rowsum.gen_biom.gen.cmm
...
ps.rowsum.uniqAnimal_biom.gen = subset_samples(ps.rowsum_biom.gen, Verschickungscode %in% listIDs_onPerAnimal_and_human)
ps.rowsum.uniqAnimal_biom.gen
# otu_table()   OTU Table:         [ 459 taxa and 368 samples ]

## MAKE VARIABLE OF DIET PREFERENCES
# How to call :
ps = ps.rowsum.uniqAnimal_biom.gen
source(file.path(path_scripts, '1_data_add.pheno.variables.R'))
ps.rowsum.uniqAnimal_biom.gen = ps
head(sample_data(ps.rowsum.uniqAnimal_biom.gen))
...
# ORDER
table(sample_data(ps.rowsum.uniqAnimal_biom.gen)$Order)
ps.rowsum.uniqAnimal_biom.gen.ord = subset_samples(ps.rowsum.uniqAnimal_biom.gen, Order == 'Artiodactyla' |
                                                     Order == 'Carnivora' |
                                                     #Order == 'Diprotodontia' |
                                                     Order == 'Perissodactyla' |
                                                     Order == 'Primates')
ps.rowsum.uniqAnimal_biom.gen.ord
table(sample_data(ps.rowsum.uniqAnimal_biom.gen.ord)$Order)

# FAMILY
table(sample_data(ps.rowsum.uniqAnimal_biom.gen)$Family)

table(sample_data(ps.rowsum.uniqAnimal_biom.gen)$Family)[
  table(sample_data(ps.rowsum.uniqAnimal_biom.gen)$Family) > 4 ]

ps.rowsum.uniqAnimal_biom.gen.fam = subset_samples(ps.rowsum.uniqAnimal_biom.gen,Family == 'Bovidae' |
...
ps.rowsum.uniqAnimal_biom.gen.fam
table(sample_data(ps.rowsum.uniqAnimal_biom.gen.fam)$Family)

# GENUS
(table(sample_data(ps.rowsum.uniqAnimal_biom.gen)$Genus))
(table(sample_data(ps.rowsum.uniqAnimal_biom.gen)$Genus)[
  table(sample_data(ps.rowsum.uniqAnimal_biom.gen)$Genus) > 4 ])

ps.rowsum.uniqAnimal_biom.gen.gen = subset_samples(ps.rowsum.uniqAnimal_biom.gen, Genus == 'Bos' |
...
ps.rowsum.uniqAnimal_biom.gen.gen
table(sample_data(ps.rowsum.uniqAnimal_biom.gen.gen)$Genus)
