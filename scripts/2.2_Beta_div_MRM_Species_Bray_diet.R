...
#################################
    ...
    # distance matrix
    dietD_d = vegan::vegdist(zoo.diet.id, metric='jaccard')
    
    options(repr.plot.height=4)
    plot(hclust(dietD_d), cex=0.4)
    
    # check by relabel with species name
    
    df.dietD_d=as.matrix(dietD_d, labels=TRUE)
    
    rownames(df.dietD_d)
    head(metadata)
    
    reord.metadata = metadata[rownames(df.dietD_d), ]
    sum(rownames(reord.metadata) == colnames(df.dietD_d)) #115

    head(reord.metadata)
    rownames(df.dietD_d) = paste(rownames(reord.metadata), reord.metadata$Tierpark, reord.metadata$Tierart..englisch.) # reord.metadata$Species_bestGuess,
    colnames(df.dietD_d) = paste(colnames(reord.metadata), reord.metadata$Tierpark, reord.metadata$Tierart..englisch.)
    
#########################
## subset microbiome and metadata to only samples with dietary data
#########################
    
    samples = rownames(zoo.diet.id)
    ps.rowsum.uniqAnimal.sub = prune_samples(samples, ps.rowsum.uniqAnimal)
    ps.rowsum.uniqAnimal.sub
    
    # phyloseq-class experiment-level object
    # otu_table()   OTU Table:         [ 602 taxa and 115 samples ]

    # generate metadata df
    metadata=as.data.frame(as.matrix(sample_data(ps.rowsum.uniqAnimal.sub)))
    
#########################
## MICROBIOME DISTANCE MATRIX
#########################

  dist_BC = vegdist(otu_table(ps.rowsum.uniqAnimal.sub), method="bray")
...
setdiff(host_tree$tip.label, as.character( sample_data(ps.rowsum.uniqAnimal.sub)$Species_bestGuess )) %>% print

# replace space with _

sample_data(ps.rowsum.uniqAnimal.sub)$Species_bestGuess = sub(" ", "_", as.character( sample_data(ps.rowsum.uniqAnimal.sub)$Species_bestGuess )  )
setdiff(host_tree$tip.label, as.character( sample_data(ps.rowsum.uniqAnimal.sub)$Species_bestGuess ) ) %>% print
...
setdiff(as.character( sample_data(ps.rowsum.uniqAnimal.sub)$Species_bestGuess ), host_tree$tip.label) %>% print
# character(0)

# Filtering host tree
to_rm = setdiff(host_tree$tip.label, as.character( sample_data(ps.rowsum.uniqAnimal.sub)$Species_bestGuess ) ) 
...
# Save filtered host tree
# SpecD = one tip per host species

# writing host tree
host_tree_file = paste0(tools::file_path_sans_ext(host_tree_file), '_16S_n15-diet_SpecD.nwk')
write.tree(host_tree_prn, file=host_tree_file)
cat('File written:', host_tree_file, '\n')
...
typeof(metadata)
dim(metadata)
metadata$Species_bestGuess = as.character( metadata$Species_bestGuess )
head(metadata)
df_nodes = df_nodes %>%
...
# save host tree
host_tree_exp_file = paste0(tools::file_path_sans_ext(host_tree_file), '-IndD.nwk')
write.tree(tree_exp, file=host_tree_exp_file)
cat('File written:', host_tree_exp_file, '\n')


## calculate patristic distance matrices for host tree

host_tree_IndD_file = host_tree_exp_file #
...
# plot
  ...
  plot(hclust(tree_IndD_d), cex=0.8)

# check
  df_tree_IndD_d =as.data.frame(as.matrix(tree_IndD_d))
  df_tree_IndD_d[1:5,1:4]

# save the distance object for host phylogeny
...
# 3_modulating_factors/2_MRM/beta_div/1_IndD.ipynb
  # reference: host_tree_dist_file = '/ebio/abt3_projects/Georg_animal_feces/data/animal/phylogeny/TT/unified_metadata_complete_20180625_rn_16S-r5k_IndD_dist.RDS'
  # patristic distance
  
  host_tree_dist_file = out_file #
  ...
  setdiff(dist_BC %>% labels, host_tree_d %>% labels) %>% print
  setdiff(host_tree_d %>% labels, dist_BC %>% labels) %>% print
...
#################################
  ...
  sum(rownames(reord.zoo.cord) == colnames(df.geo_d)) #115
  str(geo_d)
  head(reord.zoo.cord)
  ...
  tiff(paste('Hierarchical_clustering_115-diet-samples.by.location.tiff', sep=''), width = 25, height = 10, units='in', family = "Helvetica", res=150)
  plot(hclust(dist(df.geo_d)), cex=0.5)
  dev.off()
...
#################################
  ...
  # ordering by dietD_d metrix
  X = labels(dietD_d)
  host_tree_d_o = dist_mtx_order(host_tree_d, X)
  geo_d_o = dist_mtx_order(geo_d, X)
  dist_BC_o = dist_mtx_order(dist_BC, X)
  
  # check
  X %>% length %>% print                           # 115
  geo_d_o %>% labels %>% length %>% print          # 115
  host_tree_d_o %>% labels %>% length %>% print    # 115
  dist_BC_o %>% labels %>% length %>% print          # 115
  dietD_d %>% labels %>% length %>% print          # 115
...
#################################
  ...
  df_grps = metadata %>% 
    filter(NewID_Dada2Object %in% labels(dist_BC_o)) %>%
  ...
  # renaming output
  rename_df = data.frame(old_name = c('m$geo', 'm$host_phy', 'm$diet'),
                         new_name = c('Geography', 'Phylogeny', 'Diet'))
...
#####
  ...
  f =   'm$beta ~ m$geo + m$host_phy + m$diet'
  ...
  # save
  setwd(paste(path_illustrations, "MRM", sep = "/"))
  png(paste('MRM_boxplot_indD_location-and-host-and-diet_Bray.png', sep=''), width = 10, height = 15, units='in', family = "Helvetica", res=300)
  plot(p)
  dev.off()
  ...
  # Diet median coef
  mrm_res_s.Diet.coef = subset(mrm_res_s,   variable == 'Diet'  & category == 'Coef.')
  dim(mrm_res_s.Diet.coef)
  table(mrm_res_s.Diet.coef$category)
  median(mrm_res_s.Diet.coef$value)
  ...
  # evaluate the significance calculation:
  # manual
  ...
  mrm_res.diet =subset(mrm_res, variable=='m$diet')
  median(mrm_res.diet$coef_pval)
  pval_sensitivity.diet = 1 - sum(mrm_res.diet$coef_pval < 0.05) / length(mrm_res.diet$pval)
  
  data.tab = as.data.frame(cbind('var' = c('host_phy', 'geo', 'diet'), 'Coef.'=c(0, 0, 0),  'pval_median'=c(0, 0, 0), 
                                 'pval_q25'=c(0, 0, 0),   'pval_q75'=c(0, 0, 0), 'pval_sensitivity'=c(0, 0, 0)))
  ...
  data.tab$pval_sensitivity = c(pval_sensitivity.host_phy, pval_sensitivity.geo, pval_sensitivity.diet)
  data.tab$Coef. = c( median(mrm_res_s.phylogeny.coef$value), median(mrm_res_s.Geography.coef$value), median(mrm_res_s.Diet.coef$value))
  data.tab$pval_median = c(median(mrm_res.host_phy$coef_pval), median(mrm_res.geo$coef_pval),  median(mrm_res.diet$coef_pval))
  data.tab$pval_q25 = c(quantile(mrm_res.host_phy$coef_pval)[2], quantile(mrm_res.geo$coef_pval)[2], quantile(mrm_res.diet$coef_pval)[2])
  data.tab$pval_q75 = c(quantile(mrm_res.host_phy$coef_pval)[4], quantile(mrm_res.geo$coef_pval)[4], quantile(mrm_res.diet$coef_pval)[4])
  ...
  sink('MRM_perm_indD_location-and-host-and-diet_Bray.txt')
  ...
  par(mfrow=c(2,2))
  plot(density(mrm_res.host_phy$coef_pval))
  plot(density(mrm_res.geo$coef_pval))
  plot(density(mrm_res.diet$coef_pval))
  
  
#####
##### m$host_phy + m$geo 
#####  same sample size as with diet
  ...
  # save
  setwd(paste(path_illustrations, "MRM", sep = "/"))
  png(paste('MRM_boxplot_indD_location-and-host-n115_Bray.png', sep=''), width = 10, height = 15, units='in', family = "Helvetica", res=300)
  plot(p)
  dev.off()
  ...
  data.tab$pval_median = as.numeric(data.tab$pval_median)
  data.tab$pval_sensitivity = c(pval_sensitivity.host_phy, pval_sensitivity.geo, pval_sensitivity.diet)
  data.tab$Coef. = c( median(mrm_res_s.phylogeny.coef$value), median(mrm_res_s.Geography.coef$value), 'NA')
  data.tab$pval_median = c(median(mrm_res.host_phy$coef_pval), median(mrm_res.geo$coef_pval),  median(mrm_res.diet$coef_pval))
  data.tab$pval_q25 = c(quantile(mrm_res.host_phy$coef_pval)[2], quantile(mrm_res.geo$coef_pval)[2], quantile(mrm_res.diet$coef_pval)[2])
  data.tab$pval_q75 = c(quantile(mrm_res.host_phy$coef_pval)[4], quantile(mrm_res.geo$coef_pval)[4], quantile(mrm_res.diet$coef_pval)[4])
  ...
  # save
  setwd(paste(path_results, "MRM", sep = "/"))
  sink('MRM_perm_indD_location-and-host-n115_Bray.txt')
