...
ps.rowsum_glom_biom.class = readRDS(file.path(path_data, "16S_DADA2_processed/Zoo.rowsum_class.rds"))
ps.rowsum_glom_biom.class
# otu_table()   OTU Table:         [ 71 taxa and 417 samples ]

#Remove taxa 
#This protects against an OTU with small mean & trivially large C.V.
ps.rowsum.gen_biom.class.cmm = filter_taxa(ps.rowsum_glom_biom.class, function(x) sum(x > 0.00) > (0.01*length(x)), TRUE)
ps.rowsum.gen_biom.class.cmm

ps.rowsum_biom.class = ps.rowsum.gen_biom.class.cmm
...
ps.rowsum.uniqAnimal_biom.class = subset_samples(ps.rowsum_biom.class, Verschickungscode %in% listIDs_onPerAnimal_and_human)
ps.rowsum.uniqAnimal_biom.class
# otu_table()   OTU Table:         [ 54 taxa and 368 samples ]

## MAKE VARIABLE OF DIET PREFERENCES
# How to call :
ps = ps.rowsum.uniqAnimal_biom.class
source(file.path(path_scripts, '1_data_add.pheno.variables.R'))
ps.rowsum.uniqAnimal_biom.class = ps
head(sample_data(ps.rowsum.uniqAnimal_biom.class))
...
# ORDER
table(sample_data(ps.rowsum.uniqAnimal_biom.class)$Order)
ps.rowsum.uniqAnimal_biom.class.ord = subset_samples(ps.rowsum.uniqAnimal_biom.class, Order == 'Artiodactyla' |
                                                     Order == 'Carnivora' |
                                                     #Order == 'Diprotodontia' |
                                                     Order == 'Perissodactyla' |
                                                     Order == 'Primates')
ps.rowsum.uniqAnimal_biom.class.ord
table(sample_data(ps.rowsum.uniqAnimal_biom.class.ord)$Order)

# FAMILY
table(sample_data(ps.rowsum.uniqAnimal_biom.class)$Family)

table(sample_data(ps.rowsum.uniqAnimal_biom.class)$Family)[
  table(sample_data(ps.rowsum.uniqAnimal_biom.class)$Family) > 4 ]

ps.rowsum.uniqAnimal_biom.class.fam = subset_samples(ps.rowsum.uniqAnimal_biom.class,Family == 'Bovidae' |
...
ps.rowsum.uniqAnimal_biom.class.fam
table(sample_data(ps.rowsum.uniqAnimal_biom.class.fam)$Family)

# GENUS
(table(sample_data(ps.rowsum.uniqAnimal_biom.class)$Genus))
(table(sample_data(ps.rowsum.uniqAnimal_biom.class)$Genus)[
  table(sample_data(ps.rowsum.uniqAnimal_biom.class)$Genus) > 4 ])

ps.rowsum.uniqAnimal_biom.class.gen = subset_samples(ps.rowsum.uniqAnimal_biom.class, Genus == 'Bos' |
...
ps.rowsum.uniqAnimal_biom.class.gen
table(sample_data(ps.rowsum.uniqAnimal_biom.class.gen)$Genus)
