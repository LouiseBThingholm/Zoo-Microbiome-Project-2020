...
# Remove taxa not seen with count more than 0 in at least 10% of the samples. 
...
# otu_table()   OTU Table:         [ 602 taxa and 374 samples ]
# sample_data() Sample Data:       [ 374 samples by 21 sample variables ]
# tax_table()   Taxonomy Table:    [ 603 taxa by 7 taxonomic ranks ]
...
#as.character( sample_data(ps.rowsum.uniqAnimal)$Species_bestGuess )[grep('Ovis', as.character( sample_data(ps.rowsum.uniqAnimal)$Species_bestGuess ))]
...
host_tree_file = paste0(tools::file_path_sans_ext(host_tree_file), '_16S_n38_SpecD.nwk')
...
# save the distance object for host phylogeny
  ...
  out_file = paste0(tools::file_path_sans_ext(host_tree_exp_file), '_dist.RDS')
...
#################################
  ...
  sum(rownames(reord.zoo.cord) == colnames(df.geo_d)) #368
  str(geo_d)
  head(reord.zoo.cord)
  ...
  png(paste('Hierarchical_clustering_samples.by.location.png', sep=''), width = 35, height = 10, units='in', family = "Helvetica", res=150)
  plot(hclust(dist(df.geo_d)), cex=0.4)
  dev.off()
...
#################################
  ...
  dist_BC %>% labels %>% length %>% print          # 368
...
#################################
  ...
  df_grps = metadata %>% 
    filter(NewID_Dada2Object %in% labels(dist_BC)) %>%
  ...
  head(mrm_res_s)
  # It took the dataframe and combined them by rows. 'coef' and 'coef_pval' was joined into one columns named 'value'. 
  # The otehrs were just rbind(). The category column was made showing if values came from 'coef' or 'coef_pval'  
  # then the category column content was recoded.
  head(rename_df)
  rename_df$old_name = as.character(rename_df$old_name)
  rename_df$new_name = as.character(rename_df$new_name)
  sum(!mrm_res_s$variable %in% rename_df$old_name)
  
  mrm_res_s = mrm_res_s %>%
    inner_join(rename_df, c('variable'='old_name')) %>%
    dplyr::select(-variable) %>% 
    rename( c(new_name = 'variable'))    # note I added the c() to make it work
  head(mrm_res_s)
  

  # boxplot of coef and p-value
  ...
  # save
  setwd(paste(path_illustrations, "MRM", sep = "/"))
  png(paste('MRM_boxplot_perm_indD_location-and-host_Bray.png', sep=''), width = 10, height = 15, units='in', family = "Helvetica", res=300)
  plot(p)
  dev.off()
  
  head(mrm_res)
  mrm_res$variable = as.factor(mrm_res$variable)
  ...
  sink('MRM_perm_indD_location-and-host_Bray.txt')
