...
ps.rowsum_glom
# otu_table()   OTU Table:         [ 1381 taxa and 417 samples ]# otu_table()   OTU Table:         [ 448 taxa and 426 samples ]
...
#################################
    ...
    png(paste('Hierarchical_clustering_115-diet-with.species.info.png', sep=''), width = 20, height = 10, units='in', family = "Helvetica", res=300)
    plot(hclust(dist(df.dietD_d)), cex=0.6)
    dev.off()
    
#########################
## subset microbiome and metadata to only samples with dietary data
#########################
    samples = rownames(zoo.diet.sub)
...
#########################
    ...
    dist_Jaccard = vegdist(otu_table(ps.rowsum.uniqAnimal.sub), method="jaccard", binary=T)
        
   # x %>% as.vector %>% summary)       
  
  # # status on Jaccard dissimilarity matrix
...
# full host tree 
host_tree_file = '~/Dropbox/life.science/Projects/Microbiome/2019.02.21_ZOO/V4/analysis/unique_species_latin_best-guess_V4_n42.nwk'
# host tree
...
## IN reference. File written: /ebio/abt3_projects/Georg_animal_feces/data/animal/phylogeny/TT/unified_metadata_complete_20180625_rn_16S-r5k_SpecD.nwk 
...
#################################
  ...
  setwd(paste(path_illustrations, "MRM", sep = "/"))
  png(paste('Hierarchical_clustering_115-diet-samples.by.location.png', sep=''), width = 25, height = 10, units='in', family = "Helvetica", res=300)
...
#################################
  ...
  dist_Jaccard_o = dist_mtx_order(dist_Jaccard, X)
  
  # check
  X %>% length %>% print                           # 115
  geo_d_o %>% labels %>% length %>% print          # 115
  host_tree_d_o %>% labels %>% length %>% print    # 115
  dist_Jaccard_o %>% labels %>% length %>% print          # 115
...
#################################
  ...
  df_grps = metadata %>% 
    filter(NewID_Dada2Object %in% labels(dist_Jaccard_o)) %>%
...
#####
  ...
  f =   'm$beta ~ m$host_phy + m$geo + m$diet'
  ...
  # save
  setwd(paste(path_illustrations, "MRM", sep = "/"))
  png(paste('MRM_boxplot_indD_location-and-host-and-diet_Jaccard.png', sep=''), width = 10, height = 15, units='in', family = "Helvetica", res=300)
  plot(p)
  dev.off()
  ...
  sink('MRM_perm_indD_location-and-host-and-diet_Jaccard.txt')
...
#####  same sample size as with diet
  ...
  p = ggplot(mrm_res_s, aes(variable, value)) +
    ...
    theme(                                          # the facet_grid labels
      strip.text.x = element_text(
        size = 24, color = "black", face = "bold"    #bold.italic
      ),
      strip.text.y = element_text(
        size = 24, color = "black", face = "bold"
      )
  ...
  # save
  setwd(paste(path_illustrations, "MRM", sep = "/"))
  png(paste('MRM_boxplot_indD_location-and-host-n115_Jaccard.png', sep=''), width = 10, height = 15, units='in', family = "Helvetica", res=300)
  plot(p)
  dev.off()
  ...
  # save
  setwd(paste(path_results, "MRM", sep = "/"))
  sink('MRM_perm_indD_location-and-host-n115_Jaccard.txt')
  mutate(data.tab, sig_pval_sensitivity = pval_sensitivity < 0.05)
