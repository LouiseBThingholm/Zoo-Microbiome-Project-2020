...
ps.rowsum_glom_biom.spe = readRDS( file.path(path_data, "16S_DADA2_processed/Zoo.rowsum_spe.rds") )
...
ps.rowsum.gen_biom.spe.cmm = filter_taxa(ps.rowsum_glom_biom.spe, function(x) sum(x > 0.00) > (0.01*length(x)), TRUE) # 
...
ps.rowsum.uniqAnimal_biom.spe = subset_samples(ps.rowsum.gen_biom.spe.cmm, Verschickungscode %in% listIDs_onPerAnimal_and_human)
...
names(sample_data(ps.rowsum.uniqAnimal_biom.spe))
species.latin = as.character(sample_data(ps.rowsum.uniqAnimal_biom.spe)$Species..latein.)
#same as
...
sample_data(ps.rowsum.uniqAnimal_biom.spe)$Species_bestGuess = as.factor(species.latin)

# how many species have more than 1 sample:                            34
length(table(sample_data(ps.rowsum.uniqAnimal_biom.spe)$Species_bestGuess)[ 
  table(sample_data(ps.rowsum.uniqAnimal_biom.spe)$Species_bestGuess) > 1 ]) 
# who only has one:
names(table(sample_data(ps.rowsum.uniqAnimal_biom.spe)$Species_bestGuess)[ 
  table(sample_data(ps.rowsum.uniqAnimal_biom.spe)$Species_bestGuess) == 1 ]) # 
