...
ps.count = readRDS( file.path( path_data, "16S_DADA2_processed/Zoo.rarefied.count.rds") )
ps.count
# [ 56782 taxa and 417 samples  ]
length(table(sample_data(ps.count)$Species..englisch.)) # 38 host species
...
ps.count.uniqAnimal = subset_samples(ps.count, Verschickungscode %in% listIDs_onPerAnimal_and_human)
ps.count.uniqAnimal
# phyloseq-class experiment-level object
# otu_table()   OTU Table:         [  56782 taxa and 368 samples ]

length(table(sample_data(ps.count.uniqAnimal)$Species..englisch.)) # 38 host species
table(sample_data(ps.count.uniqAnimal)$Species..englisch.)
...
#tax.gen = as.data.frame(tax_table(ps.count.uniqAnimal))
meta = as.data.frame( sample_data(ps.count.uniqAnimal) )
head(meta)
table(meta$Digestion_RuminantsNonRuminants)

###ALPHA DIVERSITY
matrix_spec = otu_table(ps.count.uniqAnimal)

div_shannon_003 <- diversity(matrix_spec, index="shannon")  ### Shannon entropy
div_simpson_003 <- diversity(matrix_spec, index="simpson")  ### Simpson Diversity
div_invsimpson_003 <- diversity(matrix_spec, index="inv")   ### inverse Simpson Diversity
spec_nr_003 <- specnumber(matrix_spec,MARGIN=1)             ### Species number                  - richness, actual
div_shannon_even_003 <- div_shannon_003/log(spec_nr_003)    ### Pileous Evenness                - evenness
est_spec_003 <- estimateR(matrix_spec,rownames(matrix_spec))### Estimated species richnesses    - richness, estimated full
ACE_003 <- est_spec_003[4,]
Chao_003 <- est_spec_003[2,]
ACE_003=as.matrix(ACE_003)                                  ### ACE species richness
Chao_003=as.matrix(Chao_003)                                ### CHao1 species richness
fisher_alpha<- fisher.alpha(matrix_spec,MARGIN=1)           ### Fishers alpha (alpha diversity) --> dimensionless parameter describing the abundance distribution of speceis in fishers logseries (the higher the better)

alpha.div.matrix = as.data.frame(cbind(div_shannon_003, div_simpson_003, spec_nr_003, div_shannon_even_003, ACE_003, Chao_003))
names(alpha.div.matrix) = c('Shannon_div', 'Simpson_div', 'Richness', 'Pileous_Evenness', 'ACE', 'Chao')
head(alpha.div.matrix)

# MERGE WITH META DATA
meta$Species = as.character( meta$Species..englisch. )
head(meta)
alpha.div.matrix.meta = merge(meta, alpha.div.matrix, by=0)
dim(alpha.div.matrix.meta)


alpha.div.matrix.meta$Animal_taxonomy_Order_zoo = paste(meta$Order, ' (', meta$Tierpark, ')' ,sep = '') # 1:nrow(meta)
alpha.div.matrix.meta$Animal_taxonomy_Order = paste(meta$Order ,sep = '') # 1:nrow(meta)


#  SAVE table of alpha diversities
setwd(path_data)
write.table(alpha.div.matrix.meta, "alpha.div.matrix.meta.txt", col.names = T, row.names = T, quote = F, sep='\t')
