# microbiome at FAMILY level

# PREP DATA FOR BETADISPER ANALYSIS

# DATA
# use the subsampled -lowest-level- and rarefyed data. In DADA2, that is the ASV table.
ps.rowsum_glom_biom.fam = readRDS(file.path(path_data, "/16S_DADA2_processed/Zoo.rowsum_Family.rds"))
ps.rowsum_glom_biom.fam
# otu_table()   OTU Table:         [ 281 taxa and 417 samples ]

#Remove taxa 
#This protects against an OTU with small mean & trivially large C.V.
ps.rowsum.gen_biom.fam.cmm = filter_taxa(ps.rowsum_glom_biom.fam, function(x) sum(x > 0.00) > (0.01*length(x)), TRUE)
ps.rowsum.gen_biom.fam.cmm

ps.rowsum_biom.fam = ps.rowsum.gen_biom.fam.cmm
...
ps.rowsum.uniqAnimal_biom.fam = subset_samples(ps.rowsum_biom.fam, Verschickungscode %in% listIDs_onPerAnimal_and_human)
ps.rowsum.uniqAnimal_biom.fam
# otu_table()   OTU Table:         [ 180 taxa and 368 samples ]


## MAKE VARIABLE OF DIET PREFERENCES
# How to call :
ps = ps.rowsum.uniqAnimal_biom.fam
source(file.path(path_scripts, '1_data_add.pheno.variables.R'))
ps.rowsum.uniqAnimal_biom.fam = ps
head(sample_data(ps.rowsum.uniqAnimal_biom.fam))
...
# ORDER
table(sample_data(ps.rowsum.uniqAnimal_biom.fam)$Order)
ps.rowsum.uniqAnimal_biom.fam.ord = subset_samples(ps.rowsum.uniqAnimal_biom.fam, Order == 'Artiodactyla' |
                                                       Order == 'Carnivora' |
                                                       #Order == 'Diprotodontia' |
                                                       Order == 'Perissodactyla' |
                                                       Order == 'Primates')
ps.rowsum.uniqAnimal_biom.fam.ord
table(sample_data(ps.rowsum.uniqAnimal_biom.fam.ord)$Order)

# FAMILY
table(sample_data(ps.rowsum.uniqAnimal_biom.fam)$Family)

table(sample_data(ps.rowsum.uniqAnimal_biom.fam)$Family)[
  table(sample_data(ps.rowsum.uniqAnimal_biom.fam)$Family) > 4 ]

ps.rowsum.uniqAnimal_biom.fam.fam = subset_samples(ps.rowsum.uniqAnimal_biom.fam,Family == 'Bovidae' |
...
ps.rowsum.uniqAnimal_biom.fam.fam
table(sample_data(ps.rowsum.uniqAnimal_biom.fam.fam)$Family)

# GENUS
(table(sample_data(ps.rowsum.uniqAnimal_biom.fam)$Genus))
(table(sample_data(ps.rowsum.uniqAnimal_biom.fam)$Genus)[
  table(sample_data(ps.rowsum.uniqAnimal_biom.fam)$Genus) > 4 ])

ps.rowsum.uniqAnimal_biom.fam.gen = subset_samples(ps.rowsum.uniqAnimal_biom.fam, Genus == 'Bos' |
...
ps.rowsum.uniqAnimal_biom.fam.gen
table(sample_data(ps.rowsum.uniqAnimal_biom.fam.gen)$Genus)
