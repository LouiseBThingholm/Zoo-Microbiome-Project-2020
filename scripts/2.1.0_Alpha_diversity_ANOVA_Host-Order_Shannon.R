...
# First the overall ANOVA across all grouos at all animal-levels
    ...
    plot(density(alpha.data$Shannon_div))
    obj.order = unlist(summary(aov(alpha.data$Shannon_div ~ alpha.data$Tierpark + alpha.data$Order)))
    ...
    obj.Family = unlist(summary(aov(alpha.data$Shannon_div ~ alpha.data$Tierpark + alpha.data$Family)))
    ...
    obj.Genus = unlist(summary(aov(alpha.data$Shannon_div ~ alpha.data$Tierpark + alpha.data$Genus)))
    ...
    obj.Species = unlist(summary(aov(alpha.data$Shannon_div ~ alpha.data$Tierpark + alpha.data$Species)))
    ...
    write.table(res.across.groups, "Shannon_ANOVA_across.groups.txt", col.names = T, row.names = F, quote = F, sep='\t')

# Then groupwise comparisons comparing groups with >4 samples per group
    
    # ANIMAL-ORDER LEVEL - 5 GROUPS WITH n>4
    ...
    order_names  = names(table(as.character( alpha.data$Order) ))
    all.unique.comb = t(data.frame(combn(order_names, 2)))
    ...
    for (i in 1:6){
      ...
      obj.order = unlist(summary(aov(alpha.data.sub$Shannon_div ~ alpha.data.sub$Tierpark + alpha.data.sub$Order)))
    ...
    # save table
    setwd( file.path( path_results, "/Alpha_diversity") )
    write.table(res.across.groups, "Shannon_ANOVA_pairvise.groups_Order.txt", col.names = T, row.names = F, quote = F, sep='\t')
